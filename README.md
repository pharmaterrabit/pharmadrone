...
| `APP_PASSWORD` | cloud | Customer / Analyst login |
| `PLATFORM_ADMIN_PASSWORD` | 6D-B admin | Platform Administration login; must differ from other passwords |
| `WORKSPACE_ADMIN_PASSWORD` | 6D-B admin | organisation-scoped Workspace Administration login |
| `WORKSPACE_ADMIN_ORGANISATION_ID` | with workspace admin | durable organisation ID that limits Workspace Administration |
...
The default `app.py` now launches the approved PharmaTune enterprise Customer / Analyst experience with separate, lazy pages for discovery, profiles, human validation, case studies, source operations and system health. Opportunity Explorer filtering and pagination run at the database, audit history is loaded on demand, unsupported future modules remain honest placeholders, and the Platform Admin Console remains out of scope until Checkpoint 6D-B. The previous five-tab interface is retained as `legacy_app.py` for rollback. See `CHECKPOINT_6D_A.md`.
