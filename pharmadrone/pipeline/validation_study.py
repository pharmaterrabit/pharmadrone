...
CSV_FIELDS = [
    ...
    "company_role_difference",
    "technical_manufacturer_differs",
    "target_is_product_owner_or_sponsor",
    ...
    "source_id_verified_by_structured_source",
    "manual_audit_status",
    "external_case_study_eligible",
    "exclusion_reason",
    "clinical_trial_signal_code",
...
def _validation_row(record: dict[str, Any], rank: int) -> dict[str, Any]:
    ...
    return {
        ...
        "company_match_warning_note": annotated.get("company_match_warning_note") or "",
        "company_identity_mismatch": bool(annotated.get("company_identity_mismatch")),
        "company_role_difference": bool(annotated.get("company_role_difference")),
        "technical_manufacturer_differs": bool(annotated.get("technical_manufacturer_differs")),
        "target_is_product_owner_or_sponsor": bool(annotated.get("target_is_product_owner_or_sponsor")),
        "target_is_distributor_or_repackager_only": bool(annotated.get("target_is_distributor_or_repackager_only")),
        ...
        "source_record_present": bool(annotated.get("source_record_present")),
        "source_id_verified_by_structured_source": bool(annotated.get("source_id_verified_by_structured_source")),
        "manual_audit_status": annotated.get("manual_audit_status") or "pending manual audit",
        ...
        "clinical_trial_signal_code": annotated.get("clinical_trial_signal_code") or "",
        "clinical_trial_signal_reason": annotated.get("clinical_trial_signal_reason") or "",
        "clinical_trial_evidence_field": annotated.get("clinical_trial_evidence_field") or "",
        "clinical_trial_evidence_text": annotated.get("clinical_trial_evidence_text") or "",
        "audit_correction_note": annotated.get("audit_correction_note") or "",
...
def calculate_metrics(
    ...
    return {
        ...
        "source_id_verified_count": sum(1 for r in selected_rows if r.get("source_id_verified_by_structured_source")),
        "source_record_present_count": sum(1 for r in selected_rows if r.get("source_record_present")),
        "source_id_verified_by_structured_source_count": sum(1 for r in selected_rows if r.get("source_id_verified_by_structured_source")),
        "manual_audit_completed_count": sum(1 for r in selected_rows if _norm(r.get("manual_audit_status")) not in {"", "pending manual audit"}),
...
def build_markdown_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        f"- Structured source records present: {metrics.get('source_record_present_count', 0)}",
        f"- Source IDs matched by structured-source URL/record logic: {metrics.get('source_id_verified_by_structured_source_count', 0)}",
        f"- Manual audits completed: {metrics.get('manual_audit_completed_count', 0)}",
        ...
        "Structured-source verification is deterministic and does not mean a human audit was completed. Open each official source URL where available, or locate the authoritative source manually when the URL is blank. Complete every manual audit column in the CSV.",
